...
for i in range(row):
    ...
    # # a, b 값을 넣고 zip
    # a = range(row_memory-1,row_memory+2)
    # b = range(col_memory-1, col_memory+2)
    #
    # # 테스트용
    # memory_index = []
    # a = range(-1,+2)
    # b = range(-1, +2)
    # memory_index += list(zip(a,b))
    # print(list(memory_index))
    # # 2차원을 한 줄로 표현
    # [(a,b) for a in range(-1,2) for b in range(-1,2)]

    # # 완성!
    # memory_index = []
    # memory_index += [(a,b) for a in range(-1,2) for b in range(-1,2)]
    # print(memory_index)